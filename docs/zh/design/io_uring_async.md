...
## 5. 写路径改造:WAL + Segment 合并的有序异步提交

一个 KVCache 写涉及**三处盘上写**:Segment slot 数据、Segment bitmap、WAL 记录。
它们不是三条独立路径各自提交/各自 fsync,而是**对同一批写,组织进同一个 io_uring
提交单元,用统一的 link 链表达顺序、共享一次 fsync**。整体异步(不阻塞提交方),
但盘上落盘顺序被 link 严格保序。

### 5.1 为什么 WAL 必须最后落(与数据库直觉相反)

ParaKV 的 WAL 存的是**索引指针**(`key → 编码 offset`),不是数据本身;数据在 Segment slot 里。
所以持久化顺序必须是:

```
Segment slot 数据  →  Segment bitmap  →  WAL 记录(commit record,最后落)
```

- 若 **WAL 先落、数据后落** 而崩在中间:replay 会装回一个指向**尚未落盘/垃圾 slot** 的索引项 → 脏读。
- 因此 WAL 在这里扮演 **commit record** 角色:它落盘 ⟺ "这条写已提交、数据已在盘上、索引可信"。
  数据库通常 WAL 先行,是因为其 WAL 含数据 redo;ParaKV WAL 不含数据,故顺序相反。

### 5.2 合并提交的 link 编排

对一批(一个 tick 内到达的)写,单个 io_uring 提交单元:

```
[write slot_0][write slot_1]...[write slot_{n-1}]   ← N 个 slot 数据, 乱序并发(append-only 无依赖)
        │  (全部完成后, IO_LINK)
        ▼
[write bitmap]                                       ← 数据全落后, 落 bitmap
        │  (IO_LINK)
        ▼
[write wal_batch @ wal_tail]                         ← bitmap 后, append WAL 批(索引指针)
        │  (IO_LINK)
        ▼
[fsync DATASYNC]                                     ← 一次 fsync 覆盖以上全部
  → fsync CQE 到达后, 统一 resume 这批等 commit 的写方
```

- **摊薄**:一批 N 条写共享一次 fsync(fsync 百 µs~ms 级,是大头);slot 数据段内乱序并发打满 NVMe 队列。
- **保序**:三段之间用 `IOSQE_IO_LINK` 串成 `数据 → bitmap → WAL → fsync`;段内(N 个 slot 数据)无 link,乱序。
- **正确性检查(基本律④)**:link 是"前一节完成"非"成功",short write 会把后续 `-ECANCELED`。
  ⇒ 收割时必须逐节校验 `res == len`;任一节短写/失败,则**整批视为未提交**(不 resume 成功、不认 WAL)。
- **可见性(§5.4)**:index map 何时对外可见,取决于选"立即可见"还是"fsync 后可见"。

### 5.3 崩溃恢复(recover)语义 —— 合并方案的核心

保序异步下,任意崩溃点落在 link 链的某一节。逐点分析:

| 崩溃点 | 盘上状态 | recover 结果 | 一致? |
|---|---|---|---|
| **A** slot 数据写了一半 / bitmap 未落 / WAL 未落 | bitmap 标记该 slot 空;WAL 无记录 | slot 视为未写,index 无此项 | ✅ 该 chunk 下次 miss 重算 |
| **B** slot 数据 + bitmap 已落,**WAL 未落** | slot 在盘上 live 且数据完整,但 index 无指针 | **孤儿 slot**:live 但无 index 指向 | ⚠️ 见下 |
| **C** 全部落盘(含 WAL) | 数据完整 + WAL 有指针 | WAL replay 装回 index,指向完整 slot | ✅ |

**只有 B 需要显式处理**,且它不是错误——数据是好的,只是索引没登记。因为 WAL 是 commit
record 且最后落,B 意味着"这条写未提交";cache 语义下有两种策略:

- **策略 1(默认,最简)**:忽略孤儿 slot。它占着空间,等 compaction 扫到时(该 slot 无
  index 指向 ⇒ 视为已删)顺手回收。零额外 recover 逻辑,完全符合 cache 语义。
- **策略 2(可选,主动回收/补登记)**:recover 时对每个 segment 扫 bitmap,凡"bitmap live
  但 WAL replay 后 index 未指向"的 slot,主动清 bitmap 位回收(或读出 key 反向补登记 index)。
  代价是一次全 segment 扫描。

recover 总流程(与 §0 现状的 `Index::Open` 对齐):
1. Load snapshot(index 基线);
2. Replay WAL,遇 magic/CRC mismatch 停(torn-write),`LastGoodOffset` 截断——即"最后一批
   未完成的合并提交"被整体丢弃(对应 A/B 的 WAL 那节未落);
3. (策略 2 才有)扫 segment 处理孤儿 slot。

**关键不变式**:index 里的每个 key,其指向的 slot 必然 (a) bitmap live 且 (b) 数据完整——
因为 WAL 记录只在数据+bitmap 已落之后才落。反向不成立(存在孤儿 slot),由上面策略兜底。

### 5.4 bitmap 覆写的注意

`FlushBitmap` 现在每次 `PWrite` 整个 bitmap 区(一个对齐块反复原地改写)。O_DIRECT 下这是
高频对齐块覆写,是写路径 O_DIRECT 化最麻烦处 → 阶段一保持 buffered,O_DIRECT 留阶段二。
合并提交里 bitmap 写是 link 链的固定一节,不受此影响;仅 O_DIRECT 化时需额外处理其对齐覆写。

### 5.5 持久性语义(cache 场景放宽)

合并提交把 WAL+Segment 的 fsync 统一为 link 链末端的**一次** `IORING_FSYNC_DATASYNC`,
覆盖 slot 数据 + bitmap + WAL。可见性(index map 何时对外可见)二选一:

- **立即可见(推荐)**:写入内存 map 后立即返回可见,合并批在后台异步 fsync。崩溃丢最近一个
  提交窗口的写(那些 chunk 下次 miss 重算)。写延迟最低,最符合 cache 语义。对应 §5.3 表中
  A/B:未 fsync 的整批被 `LastGoodOffset` 截断,index 与 segment 保持一致(或留孤儿 slot 由策略兜底)。
- **fsync 后可见(保守)**:提交方阻塞等本批 fsync 的 CQE 返回,再把项装进 map。保留"已 ack
  必不丢"的耐久契约,写延迟 = 组提交周期。
- **fsync 频率**:两种模式下 fsync 都是**每合并批一次**(非每条);进一步可周期性合并多批共享
  一次 fsync,吞吐换更大的崩溃窗口——cache 语义下可放宽。
...
| **P2** | Segment+WAL 合并有序异步提交(数据→bitmap→WAL→fsync 单 link 链)+ recover 语义 | 无 | 无 | 中(顺序/持久性/recover 正确性) |
