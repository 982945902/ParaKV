...
class ExtBuild(build_ext):
    user_options = build_ext.user_options + [
        ("base-dir=", None, "base directory of ParaKV project"),
    ...
    def build_cmake_targets(
        ...
        build_args += ["--target", ext.name, "parakv"]
        subprocess.check_call(["cmake", "--build", ".", "--verbose"] + build_args, cwd=cmake_dir)

        os.makedirs(os.path.join(os.path.dirname(cmake_dir), "parakv/server/"), exist_ok=True)
        shutil.copy(
            os.path.join(extdir, product),
            os.path.join(os.path.dirname(cmake_dir), "parakv/server/"),
        )
...
class ExtBuildSingleTest(ExtBuild):
    ...
    def build_cmake_targets(
        ...
        possible_paths: list[str] = [
            ...
            os.path.join(cmake_dir, "parakv", "server", self.test_name),
        ]
...
class BuildDistWheel(bdist_wheel):
    ...
    def run(self) -> None:
        ...
        if len(ext_path) == 0:
            ...
            exit(1)
        tmp_path = os.path.join(ext_path, 'parakv')
...
def parse_arguments() -> dict[str, Any]:
    parser = argparse.ArgumentParser(
        description='Setup helper for building parakv',
...
if __name__ == "__main__":
    ...
    arch = get_cpu_arch()
    print(f"🚀 Build parakv with CPU arch: {arch}")
    ...
    setup(
        ...
        ext_modules=[CMakeExtension("parakv", "parakv/")],
